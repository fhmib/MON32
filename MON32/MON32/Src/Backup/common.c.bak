...
osStatus_t RTOS_EEPROM_Write(int16_t dev_addr, uint16_t mem_addr, uint8_t *buf, int32_t length)
{
  ...
  if ((status = osMutexAcquire(i2c2Mutex, I2C_WAIT_TIMEOUT)) != osOK) {
    THROW_LOG("Acquire mutex of i2c2 failed\n");
    return status;
  }

  if ((ret = I2cEepromWrite(dev_addr, mem_addr, buf, length)) != HAL_OK) {
    THROW_LOG("Write EEPROM failed, ret = %u, ErrorCode = %#X\n", ret, hi2c2.ErrorCode);
...
osStatus_t RTOS_EEPROM_Read(int16_t dev_addr, uint16_t mem_addr, uint8_t *buf, int32_t length)
{
  ...
  if ((ret = I2cEepromRead(dev_addr, mem_addr, buf, length)) != HAL_OK) {
    THROW_LOG("Read EEPROM failed, ret = %u, ErrorCode = %#X\n", ret, hi2c2.ErrorCode);
    if (ret == HAL_ERROR) status = osError;
    else if (ret == HAL_BUSY) status = osErrorResource;
    else if (ret == HAL_TIMEOUT) status = osErrorTimeout;
  }
...
osStatus_t RTOS_DAC5535_Write(uint8_t chan, uint16_t val)
{
  ...
  if ((status = osMutexAcquire(spi3Mutex, I2C_WAIT_TIMEOUT)) != osOK) {
    THROW_LOG("Acquire mutex of spi3 failed\n");
    return status;
  }
...
osStatus_t RTOS_DAC128S085_Write(uint8_t chan, uint16_t val, uint8_t mode)
{
  ...
  if ((status = osMutexAcquire(spi2Mutex, I2C_WAIT_TIMEOUT)) != osOK) {
    THROW_LOG("Acquire mutex of spi2 failed\n");
    return status;
  }
...
osStatus_t RTOS_ADC7953_SPI4_Read(uint8_t chan, uint16_t *val)
{
  ...
  if ((status = osMutexAcquire(spi4Mutex, I2C_WAIT_TIMEOUT)) != osOK) {
    THROW_LOG("Acquire mutex of spi4 failed\n");
    return status;
  }
...
osStatus_t RTOS_ADC7953_SPI5_Read(uint8_t chan, uint16_t *val)
{
  ...
  if ((status = osMutexAcquire(spi5Mutex, I2C_WAIT_TIMEOUT)) != osOK) {
    THROW_LOG("Acquire mutex of spi5 failed\n");
    return status;
  }
...
osStatus_t RTOS_ADC7953_SPI6_Read(uint8_t chan, uint16_t *val)
{
  ...
  if ((status = osMutexAcquire(spi6Mutex, I2C_WAIT_TIMEOUT)) != osOK) {
    THROW_LOG("Acquire mutex of spi6 failed\n");
    return status;
  }
...
osStatus_t RTOS_ADC7828_Read(uint8_t chan, uint16_t *val)
{
  ...
  if ((status = osMutexAcquire(i2c1Mutex, I2C_WAIT_TIMEOUT)) != osOK) {
    THROW_LOG("Acquire mutex of i2c1 failed\n");
    return status;
  }
  ...
  if (hal_status != HAL_OK) {
    THROW_LOG("Read ADC7828 failed, ErrorCode = %#X\n", hi2c1.ErrorCode);
    return osError;
  }
...
osStatus_t RTOS_DAC5535_Write(uint8_t chan, uint16_t val)
{
  ...
  HAL_GPIO_WritePin(SPI1_CS_GPIO_Port, SPI1_CS_Pin, GPIO_PIN_SET);
  HAL_GPIO_WritePin(SPI1_CS_GPIO_Port, SPI1_CS_Pin, GPIO_PIN_RESET);
  if (HAL_SPI_Transmit(&hspi1, buf, 3, 10) != HAL_OK) {
    THROW_LOG("Write DAC5535 failed, ErrorCode = %#X\n", hspi1.ErrorCode);
    status = osError;
  }
...
osStatus_t RTOS_ADC7953_SPI4_Read(uint8_t chan, uint16_t *val)
{
  ...
  if (hal_status != HAL_OK) {
    THROW_LOG("Read ADC7953A failed, ErrorCode = %#X\n", hspi4.ErrorCode);
    if (!Is_Flag_Set(&run_status.exp, EXP_ADC_A)) {
      THROW_LOG("ADC7953A abnormal\n");
      Set_Flag(&run_status.exp, EXP_ADC_A);
    }
    return osError;
  }
  ...
  chan_rb = rxbuf[0] >> 4;
  if (chan_rb != chan) {
    EPT("Read 1st ADC7953 failed, rChanIdx != chanIdx\n");
    THROW_LOG("Read 1st ADC7953 failed, rChanIdx != chanIdx\n");
    if (!Is_Flag_Set(&run_status.exp, EXP_ADC_A)) {
      THROW_LOG("ADC7953A abnormal\n");
      Set_Flag(&run_status.exp, EXP_ADC_A);
    }
  ...
  if (Is_Flag_Set(&run_status.exp, EXP_ADC_A)) {
    THROW_LOG("ADC7953A back to normal\n");
    Clear_Flag(&run_status.exp, EXP_ADC_A);
  }
  return status;
}
...
osStatus_t RTOS_ADC7953_SPI5_Read(uint8_t chan, uint16_t *val)
{
  ...
  if (hal_status != HAL_OK) {
    THROW_LOG("Read ADC7953B failed, ErrorCode = %#X\n", hspi5.ErrorCode);
    if (!Is_Flag_Set(&run_status.exp, EXP_ADC_B)) {
      THROW_LOG("ADC7953B abnormal\n");
      Set_Flag(&run_status.exp, EXP_ADC_B);
    }
    return osError;
  }
  ...
  chan_rb = rxbuf[0] >> 4;
  if (chan_rb != chan) {
    EPT("Read ADC7953B failed, rChanIdx != chanIdx\n");
    THROW_LOG("Read ADC7953B failed, rChanIdx != chanIdx\n");
    if (!Is_Flag_Set(&run_status.exp, EXP_ADC_B)) {
      THROW_LOG("ADC7953B abnormal\n");
      Set_Flag(&run_status.exp, EXP_ADC_B);
    }
  ...
  if (Is_Flag_Set(&run_status.exp, EXP_ADC_B)) {
    THROW_LOG("ADC7953B back to normal\n");
    Clear_Flag(&run_status.exp, EXP_ADC_B);
  }
  return status;
}
