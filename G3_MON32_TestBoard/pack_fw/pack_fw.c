...
void Cal_CRC32(uint8_t* packet, uint32_t length);
...
int main(int argc, char *argv[])
{
	...
	scanf("%s", buf);
	sprintf(d_name, "146#C122_MON32_testboard_%s.bin", buf);
	...
	buf[FW_HEAD_FW_LENGTH + 3] = (uint8_t)file_length;
	buf[FW_HEAD_CRC] = (uint8_t)(CRC32 >> 24);
	buf[FW_HEAD_CRC + 1] = (uint8_t)(CRC32 >> 16);
	buf[FW_HEAD_CRC + 2] = (uint8_t)(CRC32 >> 8);
	buf[FW_HEAD_CRC + 3] = (uint8_t)CRC32;
