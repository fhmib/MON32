...
int8_t set_switch(uint8_t argc, char **argv)
{
  ...
  if (argc == 3 && !strcasecmp(argv[0], "rxsw")) {
    ...
    switch_pos -= 1;
  } else if (argc == 3 && !strcasecmp(argv[0], "txsw")) {
    switch_channel = 0;
    switch_pos = strtoul(argv[2], NULL, 10);
    switch_pos -= 1;
  } else {
...
int8_t cmd_tosa(uint8_t argc, char **argv)
{
  if (argc == 4 && !strcasecmp(argv[1], "set")) {
    ...
    get_tosa(argc, argv);
  } else if (argc == 3 && !strcasecmp(argv[1], "thr")) {
...
int8_t set_tosa(uint8_t argc, char **argv)
{
  ...
  sscanf(argv[2], "%lf", &power_h);
  sscanf(argv[3], "%lf", &power_l);
  
  BE16_To_Buffer((uint16_t)(int16_t)(power_h * 100), txBuf);
  BE16_To_Buffer((uint16_t)(int16_t)(power_l * 100), txBuf + 2);

  ret = process_command(CMD_SET_TOSA, txBuf, 4, rBuf, &rLen);
  if (ret) {
    return ret;
  }
...
int8_t get_tosa_thr(uint8_t argc, char **argv)
{
  ...
  ret = process_command(CMD_QUERY_TOSA_THR, txBuf, 1, rBuf, &rLen);
  if (ret) {
    return ret;
  }
...
int8_t cmd_rx_pd_cali(uint8_t argc, char **argv)
{
  ...
  txBuf[0] = (uint8_t)strtoul(argv[1], NULL, 0);

  ret = process_command(CMD_RX_PD_CALI, txBuf, 1, rBuf, &rLen);
  if (ret) {
    return ret;
  }
...
int8_t cmd_tap_pd_cali(uint8_t argc, char **argv)
{
  ...
  ret = process_command(CMD_TAP_PD_CALI, txBuf, 1, rBuf, &rLen);
  if (ret) {
    return ret;
  }
  ...
  while (i--) {
    switch (*p) {
      ...
      case 0xB:
        ...
        PRINT("TX 1x32 X1 : %u(%#X)\r\n", u_val32, u_val32);
        u_val32 = (uint32_t)Buffer_To_BE16(p + 3);
        PRINT("TX 1x32 X2 : %u(%#X)\r\n", u_val32, u_val32);
        break;
      case 0xC:
        u_val32 = (uint32_t)Buffer_To_BE16(p + 1);
        PRINT("TX 1x32 Y1 : %u(%#X)\r\n", u_val32, u_val32);
        u_val32 = (uint32_t)Buffer_To_BE16(p + 3);
        PRINT("TX 1x32 Y2 : %u(%#X)\r\n", u_val32, u_val32);
        break;
      case 0xD:
        u_val32 = (uint32_t)Buffer_To_BE16(p + 1);
        PRINT("RX 1x32 X1 : %u(%#X)\r\n", u_val32, u_val32);
        u_val32 = (uint32_t)Buffer_To_BE16(p + 3);
        PRINT("RX 1x32 X2 : %u(%#X)\r\n", u_val32, u_val32);
        break;
      case 0xE:
        u_val32 = (uint32_t)Buffer_To_BE16(p + 1);
        PRINT("RX 1x32 Y1 : %u(%#X)\r\n", u_val32, u_val32);
        u_val32 = (uint32_t)Buffer_To_BE16(p + 3);
        PRINT("RX 1x32 Y2 : %u(%#X)\r\n", u_val32, u_val32);
        break;
      case 0xF:
        val32 = (int32_t)Buffer_To_BE32(p + 1);
        d_val = (double)val32 / 100;
        PRINT("TAP PD Power + tx 1x32 chan IL : %.2lf(%#X)dBm\r\n", d_val, val32);
        break;
      case 0x10:
        val32 = (int32_t)Buffer_To_BE32(p + 1);
        d_val = (double)val32 / 100;
      ...
      case 0x11:
        val32 = (int32_t)Buffer_To_BE32(p + 1);
        d_val = (double)val32 / 100;
        PRINT("TX 1x32 Current chan IL : %.2lf(%#X)dBm\r\n", d_val, val32);
        break;
      case 0x12:
        val32 = (int32_t)Buffer_To_BE32(p + 1);
        d_val = (double)val32 / 100;
...
int8_t cmd_tosa_status(uint8_t argc, char **argv)
{
  int8_t ret;
  if (argc > 1) {
    cmd_help2(argv[0]);
  ...
  ret = process_command(CMD_QUERY_TOSA_STATUS, txBuf, 0, rBuf, &rLen);
  if (ret) {
    return ret;
  }
  ...
  if (rBuf[CMD_SEQ_MSG_DATA] == 0) {
    ...
    PRINT("The tosa is switching\r\n");
  } else {
    PRINT("Unknown parameter %u\r\n", rBuf[CMD_SEQ_MSG_DATA]);
  }

  return 0;
}

int8_t cmd_set_tosa_wavelength(uint8_t argc, char **argv)
{
  uint8_t which;
  ...
  which = (uint8_t)strtoul(argv[1], NULL,0);
  txBuf[0] = which;
  return process_command(CMD_SET_TOSA_WAVELENGTH, txBuf, 1, rBuf, &rLen);
}
...
int8_t cmd_for_debug(uint8_t argc, char **argv)
{
  ...
  } else if (argc == 3 && !strcasecmp(argv[1], "txsw_io")) {
  ...
  } else if (argc == 3 && !strcasecmp(argv[1], "timer2")) {
    return debug_set_tim(argc, argv);
  ...
  } else if (argc == 3 && !strcasecmp(argv[1], "loopback")) {
...
int8_t debug_cal(uint8_t argc, char **argv)
{
  ...
  if (argc == 6 && !strncasecmp(argv[2], "sw", 2)) {
  ...
  } else if (argc == 8 && !strcasecmp(argv[2], "tosa_C98")) {
  ...
  } else if (argc == 8 && !strcasecmp(argv[2], "tosa_C122")) {
    sw_num = strtoul(argv[3], NULL, 10);
    if (sw_num < 1 || sw_num > 10) {
      PRINT("Invalid number\r\n");
      return 2;
  ...
  } else if (argc == 6 && !strcasecmp(argv[2], "pd3")) {
    num = strtoul(argv[3], NULL, 10);\
    if (num < 1 || num > 10) {
      PRINT("Invalid number\r\n");
      return 3;
    ...
    BE32_To_Buffer(val_x, txBuf + 16);
    ret = process_command(CMD_FOR_DEBUG, txBuf, 20, rBuf, &rLen);
  } else if (argc == 6 && !strcasecmp(argv[2], "pd4")) {
    num = strtoul(argv[3], NULL, 10);
    if (num < 1 || num > 10) {
      PRINT("Invalid number\r\n");
      return 3;
...
{
  if (!strcasecmp(argv[1], "timer")) {
    pro_ctl_c122 = 0;
  } else if (!strcasecmp(argv[1], "timer2")) {
    pro_ctl_c122 = 1;
  }
...
int8_t debug_get_tmp_all(uint8_t argc, char **argv)
{
  ...
  p = rBuf + CMD_SEQ_MSG_DATA + 48;
  value = switch_endian(*(uint32_t*)p);
  p = rBuf + CMD_SEQ_MSG_DATA + 48 + 4;
  temp = (int16_t)switch_endian(*(uint32_t*)p);
  PRINT("Tosa Temperature: %.2lfC, Original value: %u\r\n", (double)temp/100, value);

  p = rBuf + CMD_SEQ_MSG_DATA + 48 + 8;
  value = switch_endian(*(uint32_t*)p);
  p = rBuf + CMD_SEQ_MSG_DATA + 48 + 12;
  temp = (int16_t)switch_endian(*(uint32_t*)p);
  PRINT("TEC Voltage: %dmV, Original value: %u\r\n", temp, value);

  p = rBuf + CMD_SEQ_MSG_DATA + 48 + 16;
  value = switch_endian(*(uint32_t*)p);
  p = rBuf + CMD_SEQ_MSG_DATA + 48 + 20;
  temp = (int16_t)switch_endian(*(uint32_t*)p);
  PRINT("TEC Current: %dmA, Original value: %u\r\n", temp, value);

  p = rBuf + CMD_SEQ_MSG_DATA + 48 + 24;
  value = switch_endian(*(uint32_t*)p);
  p = rBuf + CMD_SEQ_MSG_DATA + 48 + 28;
  temp = (int16_t)switch_endian(*(uint32_t*)p);
  PRINT("LD Current: %dmA, Original value: %u\r\n", temp, value);

  p = rBuf + CMD_SEQ_MSG_DATA + 48 + 32;
  value = switch_endian(*(uint32_t*)p);
  p = rBuf + CMD_SEQ_MSG_DATA + 48 + 36;
  temp = (int16_t)switch_endian(*(uint32_t*)p);
  PRINT("LD Voltage: %dmV, Original value: %u\r\n", temp, value);

  p = rBuf + CMD_SEQ_MSG_DATA + 48 + 40;
  value = switch_endian(*(uint32_t*)p);
  p = rBuf + CMD_SEQ_MSG_DATA + 48 + 44;
  temp = (int16_t)switch_endian(*(uint32_t*)p);
  PRINT("MPD_Current: %duA, Original value: %u\r\n", temp, value);
...
int8_t debug_get_switch_channel()
{
  ...
  if (rBuf[CMD_SEQ_MSG_DATA + 1] < 32) {
    PRINT("TX channel is %u(C98)\r\n", rBuf[CMD_SEQ_MSG_DATA + 1] + 1);
  } else if (rBuf[CMD_SEQ_MSG_DATA + 1] < 64) {
    PRINT("TX channel is %u(C122)\r\n", rBuf[CMD_SEQ_MSG_DATA + 1] + 1 - 32);
  } else if (rBuf[CMD_SEQ_MSG_DATA + 1] < 65) {
    PRINT("TX channel is 33(C98)\r\n");
  } else if (rBuf[CMD_SEQ_MSG_DATA + 1] < 66) {
    PRINT("TX channel is 33(C122)\r\n");
  } else {
    PRINT("Invalid TX channel %#X\r\n", rBuf[CMD_SEQ_MSG_DATA + 1]);
  }
