...
#if 0
osStatus_t RTOS_DAC128S085_Write_Nodelay(uint8_t chan, uint16_t val, uint8_t mode)
{
  osStatus_t status;
  uint8_t buf[2];
#if 0
  if ((status = osMutexAcquire(spi2Mutex, 0)) != osOK) {
    THROW_LOG(MSG_TYPE_ERROR_LOG, "Acquire mutex of spi2 failed(Nodelay)\n");
    return status;
  }
#endif
  if (mode == DAC128S085_MODE_NORMAL) {
