...
int8_t Set_Switch(uint8_t switch_channel, uint8_t switch_pos)
{
  ...
  if (   (switch_channel == TX_SWITCH_CHANNEL && switch_pos >= 33) \
...
uint8_t debug_test_tosa(uint32_t which)
{
  osStatus_t status;
  uint16_t value;
...
uint8_t debug_set_tosa(uint16_t low, uint16_t high)
{
  ...
  status = RTOS_DAC128S085_Write(DAC128S085_TOSA_POWER_CHANNEL, high, DAC128S085_MODE_NORMAL);
  if (status != osOK) {
    return RESPOND_FAILURE;
  }
