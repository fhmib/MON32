...
osSemaphoreId_t optCpltSemaphore;                  // semaphore id
...
osMessageQueueId_t mid_LazerManager2;                // message queue id
...
osThreadId_t lazerManager2TaskHandle;
const osThreadAttr_t lazerManager2Task_attributes = {
  .name = "lazerManager2Task",
  .priority = (osPriority_t) LAZER_MANAGER_PRIORITY,
  .stack_size = 1024
};
...
void MX_FREERTOS_Init(void) {
  ...
  optCpltSemaphore = osSemaphoreNew(1U, 0U, NULL);
  if (optCpltSemaphore == NULL) {
    Set_Flag(&run_status.internal_exp, INT_EXP_INIT);
  }
  ...
  mid_LazerManager2 = osMessageQueueNew(LM_QUEUE_LENGTH, sizeof(MsgStruct), NULL);
  if (mid_LazerManager2 == NULL) {
    Set_Flag(&run_status.internal_exp, INT_EXP_INIT);
  }
  ...
  lazerManager2TaskHandle = osThreadNew(lazerManager2Task, NULL, &lazerManager2Task_attributes);
...
void isrTask(void *argument)
{
  ...
  for (;;)
  {
    ...
    if (msg.type == MSG_TYPE_FLASH_ISR) {
      ...
      osMutexRelease(swMutex);
      osMessageQueuePut(mid_LazerManager, &msg, 0U, 0U);
    } else if (msg.type == MSG_TYPE_SELF_CHECK_SWITCH_STEP_2) {
      if ((status = osMutexAcquire(swMutex, 50)) != osOK) {
        THROW_LOG(MSG_TYPE_ERROR_LOG, "Acquire mutex of sw failed\n");
      ...
      if (Is_Flag_Set(&run_status.osc_flag, OSC_TXSW_CHAN1)) {
        if (run_status.tosa_C122) {
          ...
          // C98 self-check ending, do nothing because C122 self-check starts immediately
        }
      } else {
    ...
    if (HAL_GPIO_ReadPin(SW1_BLOCK_GPIO_Port, SW1_BLOCK_Pin) == GPIO_PIN_RESET) {
      ...
      if (run_status.tx_block) {
        ...
        if (run_status.tx_switch_channel != 0xFF) {
          ...
          } else if (run_status.tx_switch_channel >= 0x20 && run_status.tx_switch_channel < 0x40 && !run_status.tosa_C122) {
            run_status.tx_switch_channel -= 0x20;
          }
...
void lazerManagerTask(void *argument)
{
  ...
  for (;;)
  {
    ...
    if (msg.type == MSG_TYPE_LAZER_ENABLE) {
      if (run_status.tosa_C122) {
        if (run_status.tosa_dst_power_high > tosa_power_high_max_thr_C122) {
          run_status.tosa_dst_power_high = tosa_power_high_max_thr_C122;
        }
        if (run_status.tosa_dst_power_low < tosa_power_low_min_thr_C122) {
          run_status.tosa_dst_power_low = tosa_power_low_min_thr_C122;
        }
      } else {
        if (run_status.tosa_dst_power_high > tosa_power_high_max_thr_C98) {
          run_status.tosa_dst_power_high = tosa_power_high_max_thr_C98;
        }
        if (run_status.tosa_dst_power_low < tosa_power_low_min_thr_C98) {
          run_status.tosa_dst_power_low = tosa_power_low_min_thr_C98;
        }
      }
      ...
      if (run_status.modulation) {
        if (run_status.tosa_C122) {
          RTOS_DAC128S085_C122_Write(DAC128S085_TEC_VALUE_CHANNEL, (run_status.tosa_high.tec_dac + run_status.tosa_low.tec_dac) / 2, DAC128S085_MODE_NORMAL);
          if (run_status.power_mode) {
            ...
            RTOS_DAC128S085_C122_Write(DAC128S085_TOSA_POWER_CHANNEL, run_status.tosa_low.tosa_dac, DAC128S085_MODE_NORMAL);
          }
        } else {
          RTOS_DAC128S085_C98_Write(DAC128S085_TEC_VALUE_CHANNEL, (run_status.tosa_high.tec_dac + run_status.tosa_low.tec_dac) / 2, DAC128S085_MODE_NORMAL);
          if (run_status.power_mode) {
            RTOS_DAC128S085_C98_Write(DAC128S085_TOSA_POWER_CHANNEL, run_status.tosa_high.tosa_dac, DAC128S085_MODE_NORMAL);
          } else {
            RTOS_DAC128S085_C98_Write(DAC128S085_TOSA_POWER_CHANNEL, run_status.tosa_low.tosa_dac, DAC128S085_MODE_NORMAL);
          }
        }
      } else {
        if (run_status.tosa_C122) {
      ...
      msg.type = MSG_TYPE_SELF_CHECK_SWITCH_STEP_2;
      msg.pbuf = NULL;
      ...
      if (run_status.tosa_C122) {
        status = RTOS_EEPROM_Read(EEPROM_ADDR, EE_CAL_LB_IL_C122 + 2, buf, 2);
      } else {
        status = RTOS_EEPROM_Read(EEPROM_ADDR, EE_CAL_LB_IL_C98 + 2, buf, 2);
      }
      ...
      osMessageQueuePut(mid_ISR, &msg, 0U, 0U);
    } else if (msg.type == MSG_TYPE_PRO_CTL_RAISING_ISR) {
      if (run_status.tosa_enable) {
        if (run_status.tosa_C122) {
          RTOS_DAC128S085_C122_Write(DAC128S085_TOSA_POWER_CHANNEL, run_status.tosa_high.tosa_dac, DAC128S085_MODE_NORMAL);
        } else {
      ...
      run_status.power_mode = 1;
    } else if (msg.type == MSG_TYPE_PRO_CTL_FALLING_ISR) {
      if (run_status.tosa_enable) {
        if (run_status.tosa_C122) {
          RTOS_DAC128S085_C122_Write(DAC128S085_TOSA_POWER_CHANNEL, run_status.tosa_low.tosa_dac, DAC128S085_MODE_NORMAL);
        } else {
...
void lazerManager2Task(void *argument)
{
  ...
  for (;;)
  {
    status = osMessageQueueGet(mid_LazerManager2, &msg, 0U, osWaitForever);
    if (status != osOK)
      continue;
    ...
    if (msg.type == MSG_TYPE_SWITCH_WAVELENGTH) {
      ...
      if (run_status.tosa_enable) {
        ...
        if (run_status.modulation) {
          ...
          THROW_LOG(MSG_TYPE_NORMAL_LOG, "Cannot start self-check in modulation mode\n");
          run_status.tosa_is_switching = 0;
          continue;
        }
    ...
    } else if (msg.type == MSG_TYPE_RUN_SELFCHECK) {
      run_status.tosa_is_switching = 1;
      Set_Flag(&run_status.osc_flag, OSC_INIT);
      Set_Flag(&run_status.osc_flag, OSC_ONGOING);
