...
osStatus_t Update_Tec_Dest_Temp(ThresholdStruct *table)
{
  ...
  while (times++ < 30 && i < 5) {
    if (run_status.tosa_C122) {
      status = RTOS_ADC7953_SPI5_Read(TEC_ADC_C122_TEC_TEMP_CHANNEL, &value);
    } else {
      status = RTOS_ADC7953_SPI5_Read(TEC_ADC_C98_TEC_TEMP_CHANNEL, &value);
    }
    if (status != osOK) {
      return status;
    }
...
uint8_t Enable_Tosa()
{
  ...
  if (run_status.tosa_C122) {
    if (tosa_table_count_C122 == 0) {
      Set_Flag(&run_status.internal_exp, INT_EXP_TOSA_DATA);
      return 4;
    }

    status = RTOS_DAC128S085_C122_Write(DAC128S085_TEC_SWITCH_CHANNEL, 4095, DAC128S085_MODE_NORMAL);
    if (status != osOK) {
      return 1;
    }
    status = RTOS_DAC128S085_C122_Write(DAC128S085_TOSA_SWITCH_CHANNEL, 4095, DAC128S085_MODE_NORMAL);
    if (status != osOK) {
      return 3;
    }
  } else {
    if (tosa_table_count_C98 == 0) {
      Set_Flag(&run_status.internal_exp, INT_EXP_TOSA_DATA);
      return 4;
    }

    status = RTOS_DAC128S085_C98_Write(DAC128S085_TEC_SWITCH_CHANNEL, 4095, DAC128S085_MODE_NORMAL);
    if (status != osOK) {
      return 1;
    }
    status = RTOS_DAC128S085_C98_Write(DAC128S085_TOSA_SWITCH_CHANNEL, 4095, DAC128S085_MODE_NORMAL);
    if (status != osOK) {
      return 3;
    }
  }
...
void update_tosa_table(void)
{
  ...
  tosa_table_count_C98 = 0;
  for (i = 0; i < TOSA_TABLE_COUNT; ++i) {
    status = RTOS_EEPROM_Read(EEPROM_ADDR, EE_CAL_TOSA1 + 16 * i, buf, 16);
    if (status != osOK) {
      tosa_table_count_C98 = 0;
      return ;
    }
    ...
    tosa_table_C98[i].tosa_dac = (uint16_t)Buffer_To_BE32(buf);
    tosa_table_C98[i].tec_dac = (uint16_t)Buffer_To_BE32(buf + 4);
    tosa_table_C98[i].tap_adc = (uint16_t)Buffer_To_BE32(buf + 8);
    val = (int32_t)Buffer_To_BE32(buf + 12);
    tosa_table_C98[i].tap_power = (double)val / 100;
    if (tosa_table_C98[i].tap_power > 10 || tosa_table_C98[i].tap_power < -20) {
      break;
    }
  }
  tosa_table_count_C98 = i;
  if (tosa_table_count_C98 > 1) {
    if (tosa_table_C98[tosa_table_count_C98 - 1].tap_power < 4) {
      tosa_power_high_max_thr_C98 = tosa_table_C98[tosa_table_count_C98 - 1].tap_power;
    } else {
      tosa_power_high_max_thr_C98 = 4;
    }
    if (tosa_table_C98[0].tap_power > -8) {
      tosa_power_low_min_thr_C98 = tosa_table_C98[0].tap_power;
    } else {
      tosa_power_low_min_thr_C98 = -8;
    }
  }

  tosa_table_count_C122 = 0;
  for (i = 0; i < TOSA_TABLE_COUNT; ++i) {
    status = RTOS_EEPROM_Read(EEPROM_ADDR, EE_CAL_TOSA2 + 16 * i, buf, 16);
    if (status != osOK) {
      tosa_table_count_C122 = 0;
      return ;
    }
    ...
    tosa_table_C122[i].tosa_dac = (uint16_t)Buffer_To_BE32(buf);
    tosa_table_C122[i].tec_dac = (uint16_t)Buffer_To_BE32(buf + 4);
    tosa_table_C122[i].tap_adc = (uint16_t)Buffer_To_BE32(buf + 8);
    val = (int32_t)Buffer_To_BE32(buf + 12);
    tosa_table_C122[i].tap_power = (double)val / 100;
    if (tosa_table_C122[i].tap_power > 10 || tosa_table_C122[i].tap_power < -20) {
      break;
    }
  }
  tosa_table_count_C122 = i;
  if (tosa_table_count_C122 > 1) {
    if (tosa_table_C122[tosa_table_count_C122 - 1].tap_power < 4) {
      tosa_power_high_max_thr_C122 = tosa_table_C122[tosa_table_count_C122 - 1].tap_power;
    } else {
      tosa_power_high_max_thr_C122 = 4;
    }
    if (tosa_table_C122[0].tap_power > -8) {
      tosa_power_low_min_thr_C122 = tosa_table_C122[0].tap_power;
    } else {
      tosa_power_low_min_thr_C122 = -8;
    }
  }
}
...
TosaCalData Get_Tosa_Data(double power)
{
  ...
  if (run_status.tosa_C122) {
    if (!tosa_table_count_C122) {
      tosa_data.tosa_dac = 0;
    } else if (tosa_table_count_C122 == 1) {
      tosa_data.tosa_dac = 0;
    } else {
      if (power < tosa_table_C122[0].tap_power || power > tosa_table_C122[tosa_table_count_C122 - 1].tap_power) {
        tosa_x1 = tosa_table_C122[0];
        tosa_x2 = tosa_table_C122[tosa_table_count_C122 - 1];
      } else {
        for (i = 1; i < tosa_table_count_C122; ++i) {
          if (power <= tosa_table_C122[i].tap_power) 
            break;
        }
        tosa_x1 = tosa_table_C122[i - 1];
        tosa_x2 = tosa_table_C122[i];
      }
      tosa_data = Cal_Tosa_Data(tosa_x1, tosa_x2, power);
    }
  } else {
    if (!tosa_table_count_C98) {
      tosa_data.tosa_dac = 0;
    } else if (tosa_table_count_C98 == 1) {
      tosa_data.tosa_dac = 0;
    } else {
      if (power < tosa_table_C98[0].tap_power || power > tosa_table_C98[tosa_table_count_C98 - 1].tap_power) {
        tosa_x1 = tosa_table_C98[0];
        tosa_x2 = tosa_table_C98[tosa_table_count_C98 - 1];
      } else {
        for (i = 1; i < tosa_table_count_C98; ++i) {
          if (power <= tosa_table_C98[i].tap_power) 
            break;
        }
        tosa_x1 = tosa_table_C98[i - 1];
        tosa_x2 = tosa_table_C98[i];
      }
      tosa_data = Cal_Tosa_Data(tosa_x1, tosa_x2, power);
    }
  }
...
{
  if (run_status.tosa_C122) {
    if (HAL_GPIO_ReadPin(TMPGD2_GPIO_Port, TMPGD2_Pin) == GPIO_PIN_RESET)
      return 0;
  } else {
...
uint8_t Cali_Power(double power_dst)
{
  ...
  do {
    ...
    if (power_tap > power_dst + 0.5 || power_tap < power_dst - 0.5) {
      ...
      if (run_status.tosa_high.tosa_dac > 0x1000 * 0.95) {
        ...
        if (run_status.tosa_C122) {
          RTOS_DAC128S085_C122_Write(DAC128S085_TEC_VALUE_CHANNEL, run_status.tosa_high.tec_dac, DAC128S085_MODE_NORMAL);
          RTOS_DAC128S085_C122_Write(DAC128S085_TOSA_POWER_CHANNEL, run_status.tosa_high.tosa_dac, DAC128S085_MODE_NORMAL);
        } else {
          RTOS_DAC128S085_C98_Write(DAC128S085_TEC_VALUE_CHANNEL, run_status.tosa_high.tec_dac, DAC128S085_MODE_NORMAL);
...
uint8_t cal_tap_pd_by_power(uint16_t *adc, double power, uint8_t which)
{
  double d_val;
  uint16_t adc1, adc2;
  ...
  if (which == 1) {
    p_table = tosa_table_C122;
    table_count = tosa_table_count_C122;
  } else {
    p_table = tosa_table_C98;
    table_count = tosa_table_count_C98;
  }
  ...
  power1 = p_table[0].tap_power;
  power2 = p_table[1].tap_power;
  if (power1 > power2) {
    dst = 1;
  ...
  offset = 1;
  if (dst) {
    while ((power2 = p_table[offset].tap_power) > d_val) {
      if (offset > table_count - 2)
        break;
      ++offset;
      power1 = power2;
    }
  ...
  adc1 = p_table[offset - 1].tap_adc;
  adc2 = p_table[offset].tap_adc;
...
uint8_t get_tap_pd_power(uint16_t *adc, double *power)
{
  ...
  if (run_status.tosa_C122) {
    status = RTOS_ADC7828_Read(TAP_PD_C122_CHANNEL, &u_val);
    if (status != osOK) {
      EPT("Read adc failed\n");
      Set_Flag(&run_status.internal_exp, INT_EXP_OS_ERR);
      return 1;
    ...
    adc1 = tosa_table_C122[0].tap_adc;
    adc2 = tosa_table_C122[1].tap_adc;
    if (adc1 > adc2) {
      dst = 1;
    } else if (adc1 < adc2) {
      dst = 0;
    ...
    offset = 1;
    if (dst) {
      while ((adc2 = tosa_table_C122[offset].tap_adc) > u_val) {
        if (offset > tosa_table_count_C122 - 2)
          break;
        ++offset;
        adc1 = adc2;
      }
    ...
    power1 = tosa_table_C122[offset - 1].tap_power;
    power2 = tosa_table_C122[offset].tap_power;
  } else {
    status = RTOS_ADC7828_Read(TAP_PD_C98_CHANNEL, &u_val);
    if (status != osOK) {
      EPT("Read adc failed\n");
      Set_Flag(&run_status.internal_exp, INT_EXP_OS_ERR);
      return 1;
    ...
    adc1 = tosa_table_C98[0].tap_adc;
    adc2 = tosa_table_C98[1].tap_adc;
    if (adc1 > adc2) {
      dst = 1;
    } else if (adc1 < adc2) {
      dst = 0;
    ...
    offset = 1;
    if (dst) {
      while ((adc2 = tosa_table_C98[offset].tap_adc) > u_val) {
        if (offset > tosa_table_count_C98 - 2)
          break;
        ++offset;
        adc1 = adc2;
      }
    ...
    power1 = tosa_table_C98[offset - 1].tap_power;
    power2 = tosa_table_C98[offset].tap_power;
  }
...
uint8_t get_rx_pd_power(uint16_t *adc, double *power)
{
  ...
  if (run_status.tosa_C122) {
    ...
    status = RTOS_ADC7828_Read(RX_PD_C122_CHANNEL, &u_val);
  } else {
    addr = EE_CAL_RX_PD_C98;
    status = RTOS_ADC7828_Read(RX_PD_C98_CHANNEL, &u_val);
  }
...
uint8_t get_tap_pd_power_2(uint16_t *adc, double *power, uint8_t pdnum)
{
  osStatus_t status;
  uint16_t u_val;
  uint16_t adc1, adc2;
...
uint8_t get_rx_pd_power_2(uint16_t *adc, double *power, uint8_t pdnum)
{
  osStatus_t status;
  uint16_t u_val;
  uint16_t addr;
...
uint8_t Get_Performance(uint8_t per_id, uint8_t *pBuf)
{
  ...
  switch (per_id) {
    case 0:
      if (run_status.tosa_C122) {
        status = RTOS_ADC7953_SPI5_Read(TEC_ADC_C122_LD_CURRENT_CHANNEL, &u_val16);
      } else {
        status = RTOS_ADC7953_SPI5_Read(TEC_ADC_C98_LD_CURRENT_CHANNEL, &u_val16);
      }
      if (status != osOK) {
        Set_Flag(&run_status.internal_exp, INT_EXP_OS_ERR);
    ...
    case 1:
      if (run_status.tosa_C122) {
        status = RTOS_ADC7953_SPI5_Read(TEC_ADC_C122_TEC_TEMP_CHANNEL, &u_val16);
      } else {
        status = RTOS_ADC7953_SPI5_Read(TEC_ADC_C98_TEC_TEMP_CHANNEL, &u_val16);
      }
      if (status != osOK) {
        Set_Flag(&run_status.internal_exp, INT_EXP_OS_ERR);
        BE32_To_Buffer(0x80000000, pBuf);
    ...
    case 2:
      if (run_status.tosa_C122) {
        status = RTOS_ADC7953_SPI5_Read(TEC_ADC_C122_TEC_CURRENT_CHANNEL, &u_val16);
      } else {
        status = RTOS_ADC7953_SPI5_Read(TEC_ADC_C98_TEC_CURRENT_CHANNEL, &u_val16);
      }
      if (status != osOK) {
        Set_Flag(&run_status.internal_exp, INT_EXP_OS_ERR);
        BE32_To_Buffer(0x80000000, pBuf);
    ...
    case 3:
      if (run_status.tosa_C122) {
        status = RTOS_ADC7953_SPI5_Read(TEC_ADC_C122_TEC_VOLTAGE_CHANNEL, &u_val16);
      } else {
        status = RTOS_ADC7953_SPI5_Read(TEC_ADC_C98_TEC_VOLTAGE_CHANNEL, &u_val16);
      }
      if (status != osOK) {
        Set_Flag(&run_status.internal_exp, INT_EXP_OS_ERR);
        BE32_To_Buffer(0x80000000, pBuf);
    ...
    case 6:
      if (run_status.tosa_C122) {
        if (run_status.tosa_enable && HAL_GPIO_ReadPin(TMPGD2_GPIO_Port, TMPGD2_Pin) == GPIO_PIN_SET) {
          BE32_To_Buffer(1, pBuf);
        } else {
          BE32_To_Buffer(0, pBuf);
        }
...
uint8_t debug_cal_tosa(uint8_t num, uint32_t tosa_dac, uint32_t tec_dac, uint32_t pd_adc, int32_t pd)
{
  ...
  if (num < 10) {
    addr = EE_CAL_TOSA1 + num * 16;
  } else if (num < 20) {
    addr = EE_CAL_TOSA2 + (num - 10) * 16;
  } else {
    return RESPOND_FAILURE;
  }
...
uint8_t debug_set_tosa(uint16_t low, uint16_t high)
{
  ...
  if (run_status.tosa_C122) {
    status = RTOS_DAC128S085_C122_Write(DAC128S085_TOSA_POWER_CHANNEL, high, DAC128S085_MODE_NORMAL);
  } else {
    status = RTOS_DAC128S085_C98_Write(DAC128S085_TOSA_POWER_CHANNEL, high, DAC128S085_MODE_NORMAL);
  }
  if (status != osOK) {
    return RESPOND_FAILURE;
  }
...
uint8_t debug_set_tec(uint16_t value)
{
  ...
  if (run_status.tosa_C122) {
    status = RTOS_DAC128S085_C122_Write(DAC128S085_TEC_VALUE_CHANNEL, value, DAC128S085_MODE_NORMAL);
  } else {
    status = RTOS_DAC128S085_C98_Write(DAC128S085_TEC_VALUE_CHANNEL, value, DAC128S085_MODE_NORMAL);
  }
  if (status != osOK) {
    return RESPOND_FAILURE;
  }
 ...
 uint8_t debug_get_tosa_tmp()
{
  ...
  status |= RTOS_ADC7953_SPI5_Read(TEC_ADC_C98_TEC_VOLTAGE_CHANNEL, &value);
  ...
  status |= RTOS_ADC7953_SPI5_Read(TEC_ADC_C98_TEC_CURRENT_CHANNEL, &value);
  temp = (double)value / 4096 * 2.5;
  ...
  status |= RTOS_ADC7953_SPI5_Read(TEC_ADC_C98_LD_CURRENT_CHANNEL, &value);
  temp = (double)value / 4096 * 2.5;
  temp = temp / 22 * 1000;
  ...
  status |= RTOS_ADC7953_SPI5_Read(TEC_ADC_C98_LD_VOLTAGE_CHANNEL, &value);
  temp = (double)value / 4096 * 2.5;
  ...
  status |= RTOS_ADC7953_SPI5_Read(TEC_ADC_C98_MPD_CURRENT_CHANNEL, &value);
  ...
  temp = Cal_Tosa_Temp(value);
  EPT("Temperature = %lf\n", temp);
  BE32_To_Buffer((uint32_t)value, resp_buf.buf + 48);
  value = (uint16_t)(int16_t)(temp * 100);
  BE32_To_Buffer((uint32_t)value, resp_buf.buf + 48 + 4);

  status |= RTOS_ADC7953_SPI5_Read(TEC_ADC_C122_TEC_VOLTAGE_CHANNEL, &value);
  temp = (double)value / 4096 * 2.5;
  temp = (temp - 1.25) / 0.25 * 1000;
  BE32_To_Buffer((uint32_t)value, resp_buf.buf + 48 + 8);
  value = (uint16_t)(int16_t)temp;
  BE32_To_Buffer((uint32_t)value, resp_buf.buf + 48 + 12);

  status |= RTOS_ADC7953_SPI5_Read(TEC_ADC_C122_TEC_CURRENT_CHANNEL, &value);
  temp = (double)value / 4096 * 2.5;
  temp = (temp - 1.25) / 0.285 * 1000;
  BE32_To_Buffer((uint32_t)value, resp_buf.buf + 48 + 16);
  value = (uint16_t)(int16_t)temp;
  BE32_To_Buffer((uint32_t)value, resp_buf.buf + 48 + 20);

  status |= RTOS_ADC7953_SPI5_Read(TEC_ADC_C122_LD_CURRENT_CHANNEL, &value);
  temp = (double)value / 4096 * 2.5;
  temp = temp / 22 * 1000;
  BE32_To_Buffer((uint32_t)value, resp_buf.buf + 48 + 24);
  value = (uint16_t)(int16_t)temp;
  BE32_To_Buffer((uint32_t)value, resp_buf.buf + 48 + 28);

  status |= RTOS_ADC7953_SPI5_Read(TEC_ADC_C122_LD_VOLTAGE_CHANNEL, &value);
  temp = (double)value / 4096 * 2.5;
  temp = (3.3 - temp * 2) * 1000;
  BE32_To_Buffer((uint32_t)value, resp_buf.buf + 48 + 32);
  value = (uint16_t)(int16_t)temp;
  BE32_To_Buffer((uint32_t)value, resp_buf.buf + 48 + 36);

  status |= RTOS_ADC7953_SPI5_Read(TEC_ADC_C122_MPD_CURRENT_CHANNEL, &value);
  temp = (double)value / 4096 * 2.5 * 1000;
  BE32_To_Buffer((uint32_t)value, resp_buf.buf + 48 + 40);
  value = (uint16_t)(int16_t)temp;
  BE32_To_Buffer((uint32_t)value, resp_buf.buf + 48 + 44);
...
uint8_t debug_get_pd(uint32_t which)
{
  ...
  if (which == 0) {
    ret = get_tap_pd_power_2(&adc, &power, 1);
  } else if (which == 1) {
    ret = get_tap_pd_power_2(&adc, &power, 2);
  } else if (which == 2) {
    ret = get_rx_pd_power_2(&adc, &power, 3);
  } else if (which == 3) {
    ret = get_rx_pd_power_2(&adc, &power, 4);
  } else {
